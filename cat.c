...
int xo, yo, radius, a, b, mouseMoving = 0, pressedButton = -1;
...
int vertexCount0 = 0;
vert vertices1[MAX]; // Array of mouse button 1 vertices
int vertexCount1 = 0;
...
void circleDraw(){
	...
	for (int i = 0; i < vertexCount1; i += 2) {
		glPointSize(1.0);
		glBegin(GL_POINTS);
		xo = vertices1[i].x;
		yo = vertices1[i].y;
		int x1 = vertices1[i + 1].x;
		int y1 = vertices1[i + 1].y;
...
void ellipseDraw(){
	...
	for (int i = 0; i < vertexCount0; i += 2) {
		glPointSize(1.0);
		glBegin(GL_POINTS);
		xo = vertices0[i].x;
		yo = vertices0[i].y;
...
void display(void) {
	...
	// Lines with right click
	glBegin(GL_LINES);
	for (int i = 0; i < vertexCount2; i+=2) {
		glVertex2i(vertices2[i].x, vertices2[i].y);
		glVertex2i(vertices2[i+1].x, vertices2[i+1].y);
	}
	glEnd();
...
void mouseHandler(int btn, int state, int x, int y)
{
	/* btn = mouse button
	   ...
	   x,y = click coordinates (origin top-left)*/
	if (btn == 0) {
		if (state == 0) {
		...
		if (state == 0) {
			vertices1[vertexCount1].x = x - WSx / 2;
			vertices1[vertexCount1++].y = WSy / 2 - y;
			vertices1[vertexCount1].x = x - WSx / 2;
			vertices1[vertexCount1].y = WSy / 2 - y;
			mouseMoving = 1;
			pressedButton = 1;
...
void mouseMotion(int x, int y)
{
	...
	if (mouseMoving == 1) {
		if (pressedButton == 0) {
			...
			vertices0[vertexCount0].y = WSy / 2 - y;
			//cout << "(" << x << "," << y << ")";
			//cout << "(" << vertices0[vertexCount0].x << "," << vertices0[vertexCount0].y << ")";
		}
